...
class DashBoardController extends Controller
{
   ...
   /* public function __construct()
    ...
    public function requestroom(Request $request)
    {
        ...
        $result=DB::table("users")->select('name')
            ->whereIn('id',function ($query) {
                $query->select('hostid')->from('posts')
                ->Where('posts.hostid','=','users.id');

            })
        ->get();
        return view('requestroom')->with('posts',$posts)->with('result',$result);
    }
    ...
    {
        $user_id=auth()->user()->id;
        $posts = Post::where([['booking', '=', 'pending'],['hostid', '=', $user_id]])->get();
